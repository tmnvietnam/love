...
class TextSparks
{
    ...
    drawParticle(particle) {
        ...
        ctx.save();
        ctx.translate(x, y);
        ctx.scale(size / 10, size / 10); // scale để điều chỉnh kích thước trái tim
        ctx.beginPath();
    ...
    drawStatic() {
        ...
        this.mask.forEach(subMask => {
            subMask.s.forEach(pos => {
                ...
                const size = baseSize * (0.7 + pulse * 0.6); // dao động nhẹ trong khoảng 70% - 130%
                ...
                // Hiệu ứng phụ nếu muốn (có thể bỏ phần này nếu muốn đơn giản)
                if (i % 2 === 0) return;
                ...
                ctx.save();
                ctx.translate(x, y);
                ctx.scale((size + padding * 2) / 10, (size + padding * 2) / 10);
                ctx.beginPath();
